...
const _snackbarReducer = createReducer<SnackbarState>(initialSnackbarState, (builder) => {
  builder
    .addCase(
      fromActions.openSnackbar,
      (state, { payload }): SnackbarState => ({ ...state, message: payload.message, open: true, severity: payload.severity }),
    )
    .addCase(fromActions.closeSnackbar, (state): SnackbarState => ({ ...state, message: '', open: false }));
});
