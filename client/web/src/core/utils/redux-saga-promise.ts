import { ActionCreatorWithPayload, ActionCreatorWithPreparedPayload, createAction, Middleware, PayloadAction, PrepareAction } from '@reduxjs/toolkit';
...
export function createPromiseAction<P = void, A = void, B = void>(prefix: string): PromiseActionCreatorWithPayload<P, A, B> {
...
export function* resolvePromiseAction<P = void, A = void, B = void>(action: PromiseAction<P, A, B>, value: A): Generator {
  yield put(action.meta.promise.resolveAction(value));
  action.meta?.promise?.resolve?.(value);
}

export function* rejectPromiseAction<P = void, A = void, B = void>(action: PromiseAction<P, A, B>, value: B): Generator {
...
export const promiseMiddleware: Middleware<unknown, AppState> = () => (next) => (action: PayloadAction<any, string, any>) => {
